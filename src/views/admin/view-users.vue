<template>
    <div>
      ...
      <v-dialog v-model="editDialog" max-width="400px">
        <v-card>
          ...
          <v-card-text>
            <v-text-field v-model="newPassword" label="New Password" type="password" :rules="[passwordRule]" />
            <v-text-field v-model="confirmPassword" label="Confirm Password" type="password" :rules="[confirmPasswordRule]" />
          </v-card-text>
          <v-card-actions>
  ...
  const filteredUsers = computed(() => {
    return users.value.filter((user) => {
      ...
      const matchesRole =
        selectedRole.value === "All" || user.role_name.toLowerCase() === selectedRole.value.toLowerCase();
